...
app.post('/api/verify-key', async (req, res) => {
    ...
    try {
        ...
        // Check 1: Key exist karti hai?
        if (keyError || !keyData) {
            return res.json({ success: false, message: "Invalid Key" });
        }

        // Check 2: Key pehle use ho chuki hai? (Optional: Agar tum one-time use chahte ho)
        if (keyData.is_used && keyData.used_by !== username) {
            return res.json({ success: false, message: "Key already used by another user" });
        }

        // Check 3: Key expire toh nahi hui?
        const now = new Date();
        if (new Date(keyData.expiry_time) < now) {
            return res.json({ success: false, message: "Key has expired" });
        }

        // 2. Key valid hai! Ab isse 'used' mark kar do (Optional)
        await supabase
            .from('access_keys')
            .update({ 
                is_used: true, 
...
app.post("/api/verify-key", async (req, res) => {
  ...
  const { data: keyData, error } = await supabase
    ...
    .select("*")
    .eq("key_code", keyCode)
    .maybeSingle(); // Changed from single() to avoid crash
  ...
  const { data: existingUser } = await supabase
    ...
    .select("username")
    .eq("username", userStr)
    .maybeSingle(); // This was the main cause of registration 'server error'
...
// ── Launch ───────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`\n🚀 Sentinel iNotes Server running on http://localhost:${PORT}`);
